<x-app-layout>
    <div class="max-w-6xl mx-auto px-6 py-8">
        {{-- Header with Actions --}}
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-4 md:mb-0">
                Edit {{ $userAlbum->album->name }} in collection
            </h1>
        </div>
        <form method="POST" action="{{ route('/editCollection', ['id' => $userAlbum->id]) }}" enctype="multipart/form-data"
              class="mt-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md space-y-8">
            @csrf


            {{-- Album Info --}}
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <h2 class="text-sm font-medium text-gray-700 dark:text-gray-300">Artist</h2>
                    <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $userAlbum->album->artist->name }}</p>
                </div>

                <div>
                    <h2 class="text-sm font-medium text-gray-700 dark:text-gray-300">Genre</h2>
                    <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $userAlbum->album->genre->name }}</p>
                </div>
            </div>

            {{-- Description --}}
            <div class="md:col-span-3">
                <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Description
                </label>
                <textarea id="description" name="description" rows="4"
                          class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700
                                 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:ring-blue-500
                                 focus:border-blue-500 sm:text-sm">{{ $userAlbum->description }}</textarea>
            </div>

            {{-- Rating --}}
            <div>
                <label for="rating" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    <span>
                        Rating:
                    </span>

                    <output class="text-gray-900 dark:text-gray-100 font-semibold text-center" id="ratingOutput"></output>
                </label>
                <div class="flex items-center space-x-4 mt-2">

                    @if($userAlbum->rating != null)
                        <input type="range" id="rating" name="rating" min="-0.25" max="5" step="0.25"
                               class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer
                                      accent-blue-600 dark:accent-blue-400" value="{{ $userAlbum->rating }}"/>
                    @else
                        <input type="range" id="rating" name="rating" min="-0.25" max="5" step="0.25"
                               class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer
                                      accent-blue-600 dark:accent-blue-400" value="-0.25"/>
                    @endif

                </div>
            </div>
            {{-- Picture --}}
            <div>
                @if($userAlbum->picture != null)
                    <div id="pictureContainer">
                        <img src="{{ asset('storage/' . $userAlbum->picture ) }}" alt="{{ $userAlbum->album->name }}"
                                 class="w-full h-48 object-contain rounded-lg mb-4" id="picture">
                        <button id="deletePicture">
                            Delete picture
                        </button>
                        <input type="hidden" name="pictureDeleted" id="pictureDeleted" value="false"/>
                    </div>



                @endif
                <div id="pictureForm">
                    <label for="picture" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Picture</label>
                    <input type="file" id="picture" name="picture" accept="image/*"
                           class="w-full mt-1 text-sm text-gray-900 dark:text-gray-100
                                  file:mr-4 file:py-2 file:px-4
                                  file:rounded-lg file:border-0
                                  file:text-sm file:font-medium
                                  file:bg-blue-600 file:text-white
                                  hover:file:bg-blue-700 transition" value="{{ asset('storage/' . $userAlbum->picture ) }}"/>
                </div>

            </div>


            {{-- Labels --}}
            <div>
                <h2 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Labels</h2>
                <div class="flex flex-wrap gap-4">
                    @foreach($labels as $label)
                        @if(in_array($label->id, $selectedLabels))

                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" id="label{{ $label->id }}" name="labels[]" value="{{ $label->id }}"
                                       class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-blue-600 dark:text-blue-400 focus:ring-blue-500 dark:focus:ring-blue-400" checked/>
                                <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $label->name }}</span>
                            </label>
                        @else

                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" id="label{{ $label->id }}" name="labels[]" value="{{ $label->id }}"
                                       class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-blue-600 dark:text-blue-400 focus:ring-blue-500 dark:focus:ring-blue-400"/>
                                <span class="text-gray-900 dark:text-gray-100 font-medium">{{ $label->name }}</span>
                            </label>
                        @endif
                    @endforeach
                </div>
            </div>

            {{-- Submit --}}
            <div>
                <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                    Edit collection
                </button>
            </div>
        </form>

        <div class="mt-4 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md space-y-8">


            {{-- Header with Actions --}}
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100 mb-4 md:mb-0">
                    Songs
                </h1>
            </div>
            {{-- Sides --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach($userAlbum->album->sides as $side)
                    <div class="space-y-4">
                        <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">
                            {{ $side->name }}
                        </h2>

                        <div class="space-y-2">
                            @foreach($side->songs->sortBy('order') as $song)
                                <div class="flex items-center p-2 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400 w-10">
                                        {{ $song->order }}.
                                    </span>
                                    <span class="text-gray-900 dark:text-gray-100">{{ $song->name }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                @endforeach
            </div>

        </div>
    </div>
<script>

	const input = document.getElementById("rating");
    const output = document.getElementById("ratingOutput");

    if(input.value != -0.25){
        output.innerText = parseFloat(input.value).toFixed(2);
    }
    else{
        output.innerText = "No Rating";
    }


    input.addEventListener('input', function (e){
        if(input.value == -0.25){
            const output = document.getElementById("ratingOutput");
            output.innerText = "No rating";
        }
        else{
            output.innerText = parseFloat(input.value).toFixed(2);
        }
    });
    var userAlbum = {!! json_encode($userAlbum->toArray()) !!};

    if(userAlbum.picture != null){
        const pictureForm = document.getElementById("pictureForm");
        pictureForm.setAttribute("hidden", "");
        const pictureContainer = document.getElementById("pictureContainer");
        pictureContainer.removeAttribute("hidden");
        const pictureDeleted = document.getElementById("pictureDeleted");

        const deletePicture = document.getElementById("deletePicture");
        deletePicture.addEventListener('click', function (e){
            e.preventDefault();
            const pictureForm = document.getElementById("pictureForm");
            pictureForm.removeAttribute("hidden");
            const pictureContainer = document.getElementById("pictureContainer");
            pictureContainer.setAttribute("hidden", "");
            const pictureDeleted = document.getElementById("pictureDeleted");
            pictureDeleted.value = "true";
        });
    }


</script>

</x-app-layout>
